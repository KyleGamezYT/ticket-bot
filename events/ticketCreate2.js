...
if(!interaction.isButton() && !interaction.isSelectMenu()) return;
      ...
      if(interaction.customId == `create_ticket${s}`) {
        ...
        .setFooter(`Ticketing by bobeebeatz.ml`, interaction.guild.iconURL())
        ...
        .setAuthor(`Ticket for: ${interaction.user.tag}`, interaction.user.displayAvatarURL(), `KyleGamezYT`)
        ...
        .addComponents([,
          ...
          .setURL(`https://github.com/KyleGamezYT/ticket-bot/issues`)
        ])
       ...
       const tcopener = interaction.guild.members.cache.get(opener.id);  interaction.channel.permissionOverwrites.edit(require('quick.db').fetch(`adminrole_${interaction.guild.id}${s}`), {
        ...
        .setFooter(`Ticketing by KyleGamezYTl\n🔷 On Shard: ${interaction.guild.shardId}`, interaction.guild.iconURL())
        ...
        .setFooter(`Ticketing by bobeebeatz.ml\n🔷 On Shard: ${interaction.guild.shardId}`, interaction.guild.iconURL())
                      if(!interaction.member.roles.cache.has(role)) {
                        return interaction.reply({ embeds: [norole], ephemeral: true})
                      }
  ...
  .setRequired(false) // If it's required or not
    ...
    showModal(modal, {
      ...
      interaction: interaction // Show the modal with interaction data.
        ...
        const embed2 = new MessageEmbed()
        .setColor(client.config.color.main)
        .setAuthor(`${interaction.user.tag} ⚡ Staff Member`, interaction.user.displayAvatarURL(), `https://bobeebeatz.ml/`)
